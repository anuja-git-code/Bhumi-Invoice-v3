...
function Invoice() {
  ...
  const handleSave = async () => {
    if (!backendUrl) {
      alert("Save is unavailable: backend not configured.");
      return;
    }
    try {
      const payload = { customer, items, totals: { total, sgst, cgst, grand } };
      const res = await fetch(`${backendUrl}/save-invoice`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
      const data = await res.json();
      res.ok ? alert("Invoice saved!") : alert("Failed: " + (data.error || "Unknown error"));
    } catch (e) {
    ...
    <div className="invoice-wrapper">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn-add:hover { background: #4e342e; }
      ...
      <div className="invoice-doc">
        ...
        <div className="inv-header">
          ...
          <div className="inv-header-right">
            ...
            <div className="inv-meta-row" style={{ fontSize: "11px", color: "#666" }}>
              Enquiry Date : -
            </div>
            <div className="inv-meta-row">
        ...
        <div className="receiver-row">
          ...
          {/* right side: empty as per invoice format */}
          <div className="receiver-right">
            <div className="receiver-section-title" style={{ visibility: "hidden" }}>.</div>
            <div className="receiver-fields" style={{ fontSize: "11.5px", color: "#555" }}>
            ...
            {items.map((item, i) => (
              <tr key={i}>
                ...
                <td>
                  ...
                  {items.length > 1 && (
                    <button className="remove-btn" onClick={() => removeRow(i)} title="Remove row">×</button>
                  )}
                </td>
              </tr>
            ))}
            {/* Blank spacer rows to look like the physical invoice */}
          ...
          <div className="totals-block">
            <table className="totals-table">
              <tbody>
                ...
                <tr>
                  <td>CGST &nbsp; 9 %</td>
                  <td>{cgst > 0 ? cgst.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "-"}</td>
                </tr>
                <tr>
                  <td>SGST &nbsp; 9 %</td>
                  <td>{sgst > 0 ? sgst.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "-"}</td>
                </tr>
                <tr>
                  <td>IGST &nbsp;&nbsp;&nbsp; %</td>
                  <td>-</td>
