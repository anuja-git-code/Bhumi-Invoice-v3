...
const EditInput = ({ value, onChange, placeholder, style = {} }) => (
  <input value={value} onChange={onChange} placeholder={placeholder} className="edit-input" style={style} />
);

function Invoice() {
  const [items, setItems] = useState([{ desc: "", hsn: "39232990", rate: "", qty: "", total: "" }]);
  const [customer, setCustomer] = useState({ name: "", address: "", state: "", stateCode: "27", gstin: "" });
  ...
  const addRow = () => setItems([...items, { desc: "", hsn: "39232990", rate: "", qty: "", total: "" }]);
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(`${SUPABASE_URL}/rest/v1/invoices`, {
        ...
        body: JSON.stringify({ customer, items, totals: { total, cgst, sgst, igst, roundOff: roundOffVal, grand } })
      });
      if (res.ok) { alert("Invoice saved successfully!"); }
      else { const err = await res.json(); alert("Error: " + (err.message || JSON.stringify(err))); }
    ...
    <div className="invoice-wrapper">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .action-btn { padding: 9px 22px; border: none; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; }
        ...
        .tax-invoice-label { border: 2px solid #1a1a1a; display: inline-block; padding: 2px 8px; font-weight: 700; font-size: 13px; margin-bottom: 4px; }
        .gst-sub { font-size: 10px; color: #444; line-height: 1.3; }
        .gst-number { margin-top: 8px; font-size: 11px; font-weight: 600; }
        .inv-header-center { flex: 1; padding: 10px 16px; text-align: center; display: flex; flex-direction: column; justify-content: center; }
        ...
        .receiver-meta-row { display: flex; border-bottom: 1px solid #1a1a1a; }
        .receiver-left { flex: 1; border-right: 1px solid #1a1a1a; }
        .receiver-section-title { background: #d0d8e4; font-size: 11px; font-weight: 700; padding: 3px 8px; text-align: center; text-transform: uppercase; border-bottom: 1px solid #1a1a1a; }
        ...
        .gstin-input { border: none; border-bottom: 1px dashed #aaa; outline: none; background: transparent; font-family: monospace; font-size: 13px; font-weight: 700; letter-spacing: 3px; width: 100%; padding: 2px 4px; text-transform: uppercase; }
        .meta-block { min-width: 210px; padding: 8px 12px; display: flex; flex-direction: column; gap: 7px; font-size: 12px; }
        .inv-meta-row { display: flex; align-items: center; gap: 4px; }
        .inv-meta-label { font-weight: 600; white-space: nowrap; min-width: 85px; }
        ...
        .items-table td { border: 1px solid #1a1a1a; padding: 4px 6px; text-align: center; vertical-align: middle; font-size: 12px; height: 40px; }
        .items-table td.desc-cell { text-align: left; }
        .bottom-section { display: flex; border-top: 1px solid #1a1a1a; border-bottom: 1px solid #1a1a1a; }
        .declaration-block { flex: 1; border-right: 1px solid #1a1a1a; padding: 8px 10px; font-size: 11px; color: #333; line-height: 1.6; min-height: 160px; }
        ...
        .totals-table td { border-bottom: 1px solid #1a1a1a; padding: 6px 10px; font-size: 12px; }
        .totals-table td:last-child { border-left: 1px solid #1a1a1a; text-align: right; font-weight: 600; min-width: 100px; }
        ...
        .remove-btn { background: none; border: none; color: #c62828; cursor: pointer; font-size: 14px; padding: 0 2px; opacity: 0.6; }
        ...
        @media print {
          ...
          .edit-input, .table-input, .roundoff-input, .gstin-input { border: none !important; }
      ...
      <div className="invoice-doc">
        ...
        <div className="inv-header">
          <div className="inv-header-left">
            ...
            <div className="gst-sub">ISSUE OF INVOICE UNDER GST 2017</div>
        ...
        <div className="receiver-meta-row">
          <div className="receiver-left">
            ...
            <div className="receiver-fields">
              ...
              <div className="field-row">
                ...
                <input className="gstin-input" maxLength={15} value={customer.gstin}
          ...
          <div className="meta-block">
            <div className="inv-meta-row"><span className="inv-meta-label">Invoice No. :</span><EditInput value={invoiceNo} onChange={e => setInvoiceNo(e.target.value)} placeholder="848" style={{ width: "60px" }} /></div>
            <div className="inv-meta-row"><span className="inv-meta-label">PO. No. :</span><EditInput value={poNo} onChange={e => setPoNo(e.target.value)} placeholder="-" style={{ width: "60px" }} /></div>
            <div className="inv-meta-row" style={{ color: "#555" }}><span className="inv-meta-label">Enquiry Date :</span>&nbsp;-</div>
            <div className="inv-meta-row"><span className="inv-meta-label">Invoice Date :</span><EditInput value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)} placeholder="DD/MM/YYYY" style={{ width: "75px" }} /></div>
          ...
          <tbody>
            {items.map((item, i) => (
              <tr key={i}>
                ...
                <td><input className="table-input" name="hsn" value={item.hsn} onChange={e => handleChange(i, e)} /></td>
            ...
            {items.length < 8 && Array.from({ length: Math.max(0, 8 - items.length) }).map((_, i) => (
              <tr key={`blank-${i}`}><td></td><td></td><td></td><td></td><td></td><td></td></tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="bottom-section">
          <div className="declaration-block">
            ...
            <div className="grand-total-words">Grand Total Rs. (In Words) {grand > 0 ? toWords(Math.round(grand)) : "—"}</div>
          </div>
          <div className="totals-block">
            <table className="totals-table">
              <tbody>
                <tr><td>Total</td><td>{total > 0 ? total.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "-"}</td></tr>
                <tr><td>CGST &nbsp; 9 %</td><td>{isMaharashtra && total > 0 ? cgst.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "-"}</td></tr>
                <tr><td>SGST &nbsp; 9 %</td><td>{isMaharashtra && total > 0 ? sgst.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "-"}</td></tr>
                <tr><td>IGST &nbsp; 18 %</td><td>{!isMaharashtra && igst > 0 ? igst.toLocaleString("en-IN", { minimumFractionDigits: 2 }) : "-"}</td></tr>
                <tr><td>Round off</td><td><input className="roundoff-input" type="number" value={roundOff} onChange={e => setRoundOff(e.target.value)} placeholder="0" /></td></tr>
